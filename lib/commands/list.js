...
async function listCommand() {
  ...
  sessions.forEach(session => {
    ...
    const labelText = session.label ? ` [${chalk.yellow(session.label)}]` : '';

    console.log(`  • ${chalk.cyan(session.sessionId.substring(0, 8))}  ${chalk.bold(session.projectName.padEnd(20))}${labelText.padEnd(20)}  ${session.aiToolDisplayName.padEnd(15)}  ${mobileIcon} ${mobileText}`);
