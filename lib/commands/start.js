...
const { getServerUrl, getApiUrl, getEnvironmentName, isLocal } = require('../config/environment');
...
async function startCommand(directory, options) {
  try {
    ...
    // Step 7b: For local mode, generate LAN-accessible server URL for mobile QR code
    let qrServerUrl = serverUrl;
    if (isLocal()) {
      const os = require('os');
      ...
      qrServerUrl = serverUrl.replace('localhost', lanIp).replace('127.0.0.1', lanIp);
      logger.debug(`CLI serverUrl: ${serverUrl}`);
      logger.debug(`QR serverUrl:  ${qrServerUrl} (LAN IP)`);
    }
