...
class WebSocketManager {
  ...
  handleClientConnected() {
    ...
    // NEW: Automatically trigger catchup for fresh connections to show history
    this.handleCatchupRequest({ type: 'catchup_request', lastSeq: 0 });
  ...
  async handleCatchupRequest(message) {
    ...
    logger.debug(`Catchup requested from seq ${lastSeq}`);
