...
class PTYManager {
  constructor(tool, workingDir, buffer) {
    ...
    this.tuiTools = ['opencode', 'kilo', 'crush', 'gemini'];
    ...
    // Mobile screen rotation debounce state
  ...
  write(data) {
    if (this.ptyProcess) {
      ...
      // Normalize smart quotes and dashes from mobile keyboards/voice dictation
      const normalizedData = data
        .replace(/[\u2018\u2019\u201A\u201B]/g, "'") // Smart single quotes
        .replace(/[\u201C\u201D\u201E\u201F]/g, '"') // Smart double quotes
        .replace(/[\u2013\u2014]/g, "-");            // En and Em dashes

      this.ptyProcess.write(normalizedData);
      return true;
    }
    return false;
  }
  ...
  resize(cols, rows) {
    ...
    // If mobile is connected, reduce width slightly to prevent cutoff on notched devices (Issue #13)
    // This provides a "safe area" margin for devices with rounded corners or notches.
    if (this.mobileConnected) {
      finalCols = Math.max(20, cols - 2);
    }

    // Local terminal resizes are applied immediately
    if (!this.mobileConnected) {
      this.ptyProcess.resize(finalCols, rows);
      logger.debug(`PTY resized to ${finalCols}x${rows}`);
      return;
    }

    // Mobile resizes (especially screen rotations) fire rapidly and can cause UI artifacting/glitching
    // in TUIs. We debounce these events to wait until the rotation animation completes.
    this.pendingResize = { cols: finalCols, rows };
    ...
    this.resizeTimer = setTimeout(() => {
      if (this.ptyProcess && this.pendingResize) {
        try {
          this.ptyProcess.resize(this.pendingResize.cols, this.pendingResize.rows);
          logger.debug(`PTY resized (debounced) to ${this.pendingResize.cols}x${this.pendingResize.rows}`);
        } catch (err) {
          logger.error(`Failed to resize PTY: ${err.message}`);
        }
        this.pendingResize = null;
      }
    }, 200); // 200ms wait ensures the rotation animation is completely finished
  }
