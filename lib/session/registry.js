...
// Save sessions registry to file - ATOMIC
function saveSessionsRegistry(registry) {
  ensureTermlyDir();

  try {
    const data = JSON.stringify(registry, null, 2);
    // Use atomically for production-safe concurrent writes
    atomically.writeFileSync(SESSIONS_FILE, data, 'utf8');
    logger.debug('Sessions registry saved atomically');
  } catch (err) {
...
// Get session by directory (and optionally label or tool key)
function getSessionByDirectory(workingDir, options = {}) {
  const registry = loadSessionsRegistry();
  return registry.sessions.find(
    s => s.workingDir === workingDir && 
         s.status === 'running' &&
         (!options.label || s.label === options.label) &&
         (!options.toolKey || s.aiTool === options.toolKey)
  );
}
